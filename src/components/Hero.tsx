...
export default function Hero() {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-start md:justify-center px-6 text-center pt-20 md:pt-0 lg:pt-12 gap-0 md:gap-8 lg:gap-12">
          ...
          <form
            ...
            className="hidden md:block w-full max-w-full md:max-w-[60%] lg:max-w-[400px] md:relative mb-0 md:mb-0 px-4 md:px-0 pb-4 md:pb-0"
          >
      ...
      <form
        onSubmit={handleSubmit}
        className={`md:hidden w-full max-w-full fixed bottom-0 left-0 right-0 mb-0 px-4 pb-4 z-50`}
      >
        <div className={`bg-white ${isSubmitted ? "rounded-lg" : "rounded-full"} px-4 md:px-6 py-3 md:py-4 flex items-center gap-3 shadow-[0_0_30px_rgba(225,125,24,0.3)] transition-all duration-300 z-50 md:z-auto`}>
          {isSubmitted ? (
            <span className={`${spaceMono.variable} font-space-mono text-[#1C1C1C] uppercase text-sm w-full text-center z-50 md:z-auto`}>
              INVITE REQUESTED
            </span>
          ) : (
            <>
